...
class kitchenMenu {
  constructor({ btnContainer, sectionContainer, data, allText = "All" }) {
    ...
    this.categories = [allText, ...new Set(data.map((i) => i.category))];
  ...
  createButtons() {
    this.categories.map((category) => {
      ...
      btn.addEventListener("click", (e) => {
        ...
        const filterMenu = this.data.filter(
          (item) => item.category === category || category === this.allText
        );

        debugger;
        this.createMenu(filterMenu);
      });
  ...
  createMenu(data) {
    this.sectionContainer.innerHTML = data
